...
const schema = z.object({
  ...
  analysis: z.object({
    topic: z.string(),
    facts: z.record(z.string(), z.union([z.string(), z.array(z.string())])),
    title: z.string(),
...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from("content_items")
    .select("id, title, caption, status, media_asset_id, created_at")
...
export async function POST(request: Request) {
  ...
  await recordAuditLog({
    ...
    entityId: data.id,
    description: `Created content draft "${analysis.title.slice(0, 50)}"`,
    status: "SUCCESS",
  });

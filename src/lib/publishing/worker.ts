...
export type WorkerStore = {
  ...
  loadTarget(targetId: string): Promise<{ platformSlug: string; accessToken: string; content: Record<string, unknown>; media?: { storagePath: string; mimeType: string } }>;
...
export async function processPublishingJob(jobId: string, store: WorkerStore, adapters: Record<string, PlatformAdapter>) {
  ...
  try { const result = await adapter.publish({ accessToken: target.accessToken, content: target.content, media: target.media }); const verified = result.confirmed && await adapter.verify(result); if (!verified) throw new Error("Platform response could not be verified."); await store.markPublished(jobId, result.externalPostId, result.externalUrl); await store.log(jobId, "published", result.statusCode); }
